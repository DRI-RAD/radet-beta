...
class Collection:
    ...
    def __init__(
        ...
        """Earth Engine based RADET Image Collection
        ...
        if model_args and "et_reference_factor" in model_args.keys() and model_args["et_reference_factor"]:
        ...
        if (
            model_args
            and "et_reference_resample" in model_args.keys()
            and model_args["et_reference_resample"]
            and model_args["et_reference_resample"].lower() not in resample_methods
        ):
            raise ValueError("unsupported et_reference_resample method")
        ...
        if (
            not type(self.cloud_cover_max) is int
            and not type(self.cloud_cover_max) is float
            and not utils.is_number(self.cloud_cover_max)
        ):
    ...
    def _build(self, variables=None, start_date=None, end_date=None):
        ...
        for coll_id in self.collections:
            ...
            if coll_id in self._landsat_c2_sr_collections:
                ...
                if (
                    self.filter_args is None
                    or not isinstance(self.filter_args, dict)
                    or coll_id not in self.filter_args.keys()
                ):
                    pass
    ...
    def interpolate(
        ...
        **kwargs,
        ...
        if ("et_reference" in variables) or ("et_fraction" in variables):
            if "et_reference_source" in kwargs.keys() and kwargs["et_reference_source"] is not None:
                self.model_args["et_reference_source"] = kwargs["et_reference_source"]
            if "et_reference_band" in kwargs.keys() and kwargs["et_reference_band"] is not None:
                self.model_args["et_reference_band"] = kwargs["et_reference_band"]
            if "et_reference_factor" in kwargs.keys() and kwargs["et_reference_factor"] is not None:
                self.model_args["et_reference_factor"] = kwargs["et_reference_factor"]
                if "et_reference_resample" in kwargs.keys() and kwargs["et_reference_resample"] is not None:
            ...
            for et_reference_param in ["et_reference_source", "et_reference_band", "et_reference_factor"]:
                if (
                    et_reference_param not in self.model_args.keys()
                    or not self.model_args[et_reference_param]
                ):
            ...
            #     )
                ...
                raise ValueError(f'unsupported et_reference_source: {self.model_args["et_reference_source"]}')

            # Scale reference ET images (if necessary)
            # CGM - Resampling is not working correctly so not including for now
            if self.model_args["et_reference_factor"] and (self.model_args["et_reference_factor"] != 1):

                def et_reference_adjust(input_img):
                    return input_img.multiply(self.model_args["et_reference_factor"]).set(
        ...
        if "count" in variables:
            ...
            aggregate_coll = aggregate_coll.merge(
                ee.Image.constant(0).rename(["mask"]).set({"system:time_start": ee.Date(start_date).millis()})
            )
        ...
        scene_coll = ee.ImageCollection(
            ee.Join.saveFirst(matchKey="target_img", ordering="system:time_start", ascending=False).apply(
                primary=scene_coll.filterDate(interp_start_date, interp_end_date),
        ...
        def aggregate_image(agg_start_date, agg_end_date, date_format):
            ...
            if ("et_reference" in variables) or ("et_fraction" in variables):
                ...
                if self.model_args["et_reference_resample"] and (
                    ...
                    et_reference_img = et_reference_img.setDefaultProjection(
                        daily_et_ref_coll.first().projection()
            ...
            if "ndvi" in variables:
                ndvi_img = daily_coll.filterDate(agg_start_date, agg_end_date).mean().select(["ndvi"]).float()
